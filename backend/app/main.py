...
app=FastAPI(title='IronCoach API',version='0.5.1')
...
@app.get('/health')
def health(): return {'status':'ok','service':'ironcoach-api','version':'0.5.1'}
...
def bootstrap():
    ...
    try:
        ...
        access=token(u.id)
        r=JSONResponse({'ok':True,'single_user':True,'access_token':access,'token_type':'bearer'}); set_session(r,u.id); return r
    finally: db.close()
